 ...
 */
	public function testSecuredInputCustomName() {
		...
		$this->Form->input('text_input', array(
			'name' => 'data[Option][Generals.default_role]',
		));
		$expected = array('Option.Generals.default_role');
		$this->assertEquals($expected, $this->Form->fields);

		$this->Form->input('select_box', array(
			'name' => 'data[Option][Generals.select_role]',
			'type' => 'select',
			'options' => array(1, 2),
		));
		$expected = array('Option.Generals.default_role', 'Option.Generals.select_role');
		$this->assertEquals($expected, $this->Form->fields);
	}
